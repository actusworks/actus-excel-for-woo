(()=>{var N=window.jQuery||jQuery,_e=class d{constructor(e={}){this.options={title:"",content:"",size:"medium",closeButton:!0,closeOnOverlay:!1,closeOnEscape:!0,animation:"fade",buttons:[],className:"",onOpen:null,onClose:null,beforeClose:null,focusTrap:!0,...e},this.isOpen=!1,this.$modal=null,this.$overlay=null,this.previousFocus=null,this.id=`modal-${Date.now()}-${Math.random().toString(36).substr(2,9)}`,this.create(),this.handleKeyDown=this.handleKeyDown.bind(this),this.handleOverlayClick=this.handleOverlayClick.bind(this)}create(){let e=`actus-modal-${this.options.size}`,t=`actus-modal-animation-${this.options.animation}`,s=this.options.className?this.options.className:"";this.$overlay=N(`
		...
		`),this}destroy(){this.close(),N(document).off(`keydown.modal-${this.id}`),setTimeout(()=>{this.$overlay&&this.$overlay.remove(),this.$modal=null,this.$overlay=null},300)}static message(e,t,s="",a={}){let{errorMessage:i="",details:o={},points:r=[],helpText:n="Please try again or contact support if the problem persists.",buttonText:c=null,button2:l={},onClose:u=null}=a,h="",p="",m="OK";switch(e){case"error":m="Close",p=t||"Error",h=`
				...
				`;break;case"success":p=t||"Success";let f=Object.keys(o).length>0?`<div class="acex-success-details">
						${Object.entries(o).map(([S,L])=>`<p><strong>${S}:</strong> ${L}</p>`).join("")}
					</div>`:"";h=`
				...
				`;break;case"info":p=t||"Information";let T=Object.keys(o).length>0?`<div class="acex-info-details">
						${Object.entries(o).map(([S,L])=>`<p><strong>${S}:</strong> ${L}</p>`).join("")}
					</div>`:"";h=`
				...
				`,size:"small",buttons:[{text:"Cancel",class:"actus-btn",onClick:n=>{i=!0,a(null),n.destroy()}},{text:"OK",class:"actus-btn-primary",onClick:n=>{let c=n.getBody().find(`#${o}`).val();i=!0,a(c),n.destroy()}}],onOpen:n=>{setTimeout(()=>{n.getBody().find(`#${o}`).focus().select()},150)},onClose:()=>{i||a(null),r.destroy()}}).open()})}},x=_e;var qt=window.jQuery||jQuery,ye=window.acex_phpDATA||{};async function j(d){if(!d)return console.error("Name is required"),null;if(ye["product_"+d])return ye["product_"+d];try{let e=new FormData;e.append("action","acex_get_data"),e.append("name",d),e.append("nonce",acex_phpDATA?.nonce||"");let t=await fetch(acex_phpDATA.ajax_url,{method:"POST",body:e});if(!t.ok)throw new Error(`HTTP error! status: ${t.status}`);let s=await t.json();return s.success?(ye["product_"+d]=s.data,ye["product_"+d]):(console.error("Error from server:",s.data?.message||"Unknown error"),[])}catch(e){throw console.error(`Error fetching data for '${d}':`,e),e}}async function xe(d){if(!d)return console.error("Option name is required"),null;try{let e=new FormData;e.append("action","acex_get_option"),e.append("name",d),e.append("nonce",acex_phpDATA?.nonce||"");let t=await fetch(acex_phpDATA.ajax_url,{method:"POST",body:e});if(!t.ok)throw new Error(`HTTP error! status: ${t.status}`);let s=await t.json();return s.success?s.data.options:(console.error("Error from server:",s.data?.message||"Unknown error"),null)}catch(e){throw console.error(`Error loading option '${d}':`,e),e}}async function B(d,e){if(!d)return console.error("Option name is required"),null;try{let t=new FormData;t.append("action","acex_save_option"),t.append("name",d),t.append("value",JSON.stringify(e)),t.append("nonce",acex_phpDATA?.nonce||"");let s=await fetch(acex_phpDATA.ajax_url,{method:"POST",body:t});if(!s.ok)throw new Error(`HTTP error! status: ${s.status}`);let a=await s.json();return a.success?a.data:(console.error("Error from server:",a.data?.message||"Unknown error",a.data),null)}catch(t){throw console.error(`Error saving option '${d}':`,t),t}}async function it(d=3){if(!d)return null;try{let e=new FormData;e.append("action","acex_get_preview_products"),e.append("count",d),e.append("nonce",acex_phpDATA?.nonce||"");let t=await fetch(acex_phpDATA.ajax_url,{method:"POST",body:e});if(!t.ok)throw new Error(`HTTP error! status: ${t.status}`);let s=await t.json();return s.success?s.data.products:(console.error("Error from server:",s.data?.message||"Unknown error"),null)}catch(e){throw console.error("Error fetching preview products:",e),e}}async function ot(d){if(!d||typeof d!="object")return null;try{let e=new FormData;e.append("action","acex_create_taxonomies"),e.append("data",JSON.stringify(d)),e.append("nonce",acex_phpDATA?.nonce||"");let t=await fetch(acex_phpDATA.ajax_url,{method:"POST",body:e});if(!t.ok)throw new Error(`HTTP error! status: ${t.status}`);let s=await t.json();return s.success||console.error("Error from server:",s.data?.message||"Unknown error"),s.data}catch(e){throw console.error("Error saving full product data:",e),e}}async function rt(d,e={}){let{chunkSize:t=50,onProgress:s,_data:a}=e;if(!d||!Array.isArray(d)||d.length===0)return console.error("Products must be a non-empty array"),null;if(d.length<=t)return await tt(d,a);let i=[];for(let r=0;r<d.length;r+=t)i.push(d.slice(r,r+t));let o={success:!0,total:d.length,successful:0,failed:0,warnings:0,created:0,updated:0,results:[]};for(let r=0;r<i.length;r++)try{let n=await tt(i[r],a);if(n&&n.success){o.successful+=n.successful,o.failed+=n.failed,o.created+=n.created,o.updated+=n.updated,o.results.push(...n.results);let c=n.results.filter(l=>l.warnings&&l.warnings.length>0).length;o.warnings+=c}else o.failed+=i[r].length;if(s){let c=(r+1)*t;s(Math.min(c,d.length),d.length,n)}}catch(n){console.error(`Error processing chunk ${r+1}:`,n),o.failed+=i[r].length}return o}async function tt(d,e){try{let t=new FormData;t.append("action","acex_save_multiple_products"),t.append("products_data",JSON.stringify(d)),t.append("_data",JSON.stringify(e)),t.append("nonce",acex_phpDATA?.nonce||"");let s=await fetch(acex_phpDATA.ajax_url,{method:"POST",body:t});if(!s.ok)throw new Error(`HTTP error! status: ${s.status}`);let a=await s.json();if(a.success){let i=a.data.summary;return{success:!0,total:i.total,successful:i.successful,failed:i.failed,created:i.created,updated:i.updated,results:a.data.results}}else return console.error("Error from server:",a.data?.message||"Unknown error"),{success:!1,total:d.length,successful:0,failed:d.length,results:[{error:!0,message:a.data?.message||"Unknown error"}]}}catch(t){throw console.error("Error saving batch:",t),t}}async function ne(d,e=!1,t=20,s=1,a=!1){if(!d||typeof d!="object")return console.error("Query must be an object"),null;let i=JSON.parse(JSON.stringify(d));delete i.description;try{a||(a="");let o=new FormData;o.append("action","acex_query"),o.append("limit",t),o.append("page",s),o.append("images",a),o.append("query",JSON.stringify(i)),o.append("nonce",acex_phpDATA?.nonce||""),e&&o.append("count",e);let r=await fetch(acex_phpDATA.ajax_url,{method:"POST",body:o});if(!r.ok)throw new Error(`HTTP error! status: ${r.status}`);let n=await r.json();return n.success?e?n.data.count:n.data:(console.log("data",n),console.error("Error from server:",n.data?.message||"Unknown error"),null)}catch(o){throw console.error("Error getting product count:",o),o}}async function nt(d,e){try{let t=new FormData;t.append("action","acex_validate_import"),t.append("products",JSON.stringify(d)),t.append("keys",e),t.append("nonce",acex_phpDATA?.nonce||"");let s=await fetch(acex_phpDATA.ajax_url,{method:"POST",body:t});if(!s.ok)throw new Error(`HTTP error! status: ${s.status}`);let a=await s.json();return a.success?a.data||[]:(console.error("Error from server:",a.data?.message||"Unknown error"),null)}catch(t){throw console.error("Error validating products:",t),t}}var st=[];function ae(d,...e){let t=["check","export"],s=at(!0);t.includes(d)||(console.groupCollapsed("%c"+d.padStart(10),"background: darkorange; color: white; padding: 2px 4px;",...e),s=at(),console.groupEnd()),st.push(["["+d+"]",...e,...s]),window.LOGS=st}function at(d=!1){let t=new Error().stack.split(`
...
`))),typeof m=="object"&&m!==null&&(m=JSON.stringify(m,null,2)),p.key.includes("slug")&&(m=decodeURIComponent(m||"")),typeof m=="string"&&/<\/?[a-z][\s\S]*>/i.test(m)&&(m=I("<div>").text(m).html()),m=`<div class="acex-field-value">${m}</div>`,I("<td>").html(m).appendTo(h)}),c++,c>=l)return!1})}removeDragHandlers(){if(!this.dragHandlers||!this.dragHandlers.$headers)return;let e=this.dragHandlers.$headers;e.off("dragstart dragover dragenter dragleave drop dragend"),e.off(".acex-columns"),this.dragHandlers=null}makeDraggable(e,t){let s=null,a=null,i=e.find("th[draggable]");this.dragHandlers={$headers:i},i.on("dragstart.acex-columns",o=>{a=o.currentTarget,s=parseInt(I(a).attr("data-column-index")),I(a).addClass("dragging"),o.originalEvent.dataTransfer.effectAllowed="move",o.originalEvent.dataTransfer.setData("text/html",a.innerHTML)}),i.on("dragover.acex-columns",o=>{o.preventDefault(),o.originalEvent.dataTransfer.dropEffect="move";let r=I(o.currentTarget);return a!==o.currentTarget&&r.addClass("drag-over"),!1}),i.on("dragenter.acex-columns",o=>{let r=I(o.currentTarget);a!==o.currentTarget&&r.addClass("drag-over")}),i.on("dragleave.acex-columns",o=>{I(o.currentTarget).removeClass("drag-over")}),i.on("drop.acex-columns",o=>{o.stopPropagation(),o.preventDefault();let r=I(o.currentTarget);r.removeClass("drag-over");let n=parseInt(r.attr("data-column-index"));return s!==null&&s!==n&&(this.reorderColumns(s,n),this.create(this.template)),!1}),i.on("dragend.acex-columns",o=>{I(o.currentTarget).removeClass("dragging"),i.removeClass("drag-over"),s=null,a=null})}reorderColumns(e,t){let[s]=this.template.columns.splice(e,1);this.template.columns.splice(t,0,s),console.log("Columns reordered:",this.template.columns.map(a=>a.header)),this.$container&&this.$container.trigger("columnsReordered",[this.template.columns])}},G=new Te;var Pt=[{query_name:"\u{1F4E6} All Published",status:["publish"],description:"All published products in the store",_system:!0},{query_name:"\u{1F195} Recently Added",status:["publish"],date_from:new Date(Date.now()-2592e6).toISOString().split("T")[0],description:"Products added in the last 30 days",_system:!0},{query_name:"\u{1F504} Recently Updated",date_modified_from:new Date(Date.now()-6048e5).toISOString().split("T")[0],description:"Products modified in the last 7 days",_system:!0},{query_name:"\u{1F570}\uFE0F Old Products",date_created_to:new Date(Date.now()-31536e6).toISOString().split("T")[0],description:"Products older than 1 year",_system:!0},{query_name:"\u{1F4A4} Stale Products",date_modified_to:new Date(Date.now()-15552e6).toISOString().split("T")[0],description:"Products not updated in 6+ months",_system:!0},{query_name:"\u{1F6D2} Overstocked",manage_stock:"yes",min_stock:100,description:"Products with more than 100 units in stock",_system:!0,p:!0},{query_name:"\u{1F4C9} Low Stock Alert",status:["publish"],stock_status:["instock"],manage_stock:"yes",max_stock:10,description:"Products with low stock (10 or less)",_system:!0,p:!0},{query_name:"\u26A0\uFE0F Out of Stock",status:["publish"],stock_status:["outofstock"],description:"Products currently out of stock",_system:!0,p:!0},{query_name:"\u{1F4B0} On Sale",status:["publish"],on_sale:!0,description:"Products currently on sale",_system:!0,p:!0},{query_name:"\u23F3 Expired Sales",date_on_sale_to:new Date().toISOString().split("T")[0],description:"Products with sale dates that have ended",_system:!0,p:!0},{query_name:"\u2B50 Featured",status:["publish"],featured:!0,description:"Products marked as featured",_system:!0},{query_name:"\u2B50 With Reviews",template:"products_with_reviews",description:"Products that have customer reviews",_system:!0},{query_name:"\u2B50 High Rated",template:"high_rated_products",description:"Products with an average rating of 4.0 or higher",_system:!0},{query_name:"\u2696\uFE0F Heavy Products",template:"heavy_products",description:"Products with a weight greater than 50kg. Important for shipping considerations.",_system:!0},{query_name:"\u2757 Incomplete Products",template:"incomplete_products",description:"Products missing critical data (no price, image, or SKU).",_system:!0,p:!0},{query_name:"\u274C Without Price",template:"products_no_price",description:"Products missing a regular price.",_system:!0,p:!0},{query_name:"\u274C Without Categories",template:"uncategorized_products",description:"Products not assigned to any category. Important for organization and navigation.",_system:!0,p:!0},{query_name:"\u274C Without Featured Image",template:"no_featured_image",description:"Products missing a featured image. Important for visual appeal.",_system:!0,p:!0},{query_name:"\u274C Without Description",template:"no_description",description:"Products missing a description. Important for SEO and conversions.",_system:!0},{query_name:"\u274C Without Weight",template:"no_weight",description:"Products missing a weight attribute. Can affect shipping calculations.",_system:!0},{query_name:"\u274C Without Dimensions",template:"no_dimensions",description:"Products missing dimensions (length, width, height). Important for shipping and display.",_system:!0}],Se=Pt;var E=window.jQuery||jQuery,be=window.acex_phpDATA||{},Ae,Ee=class{constructor(e,t,s){if(this.$container=null,this.$input=e||E(),this.$target=t||E(),this.name=s||"",this.$list=E(),this.listCurrent=0,this.result=[],this.selected=[],this.callback=null,this.$input.length){this.$container=this.$input.parent(),this.$input.attr("autocomplete","off"),this.name=this.name||this.$input.attr("name")||this.$container.attr("name")||"search_product";let a=this.$container.css("position");["relative","absolute","fixed"].includes(a)||this.$container.css("position","relative"),this.$hidden=E('<input type="hidden" />').attr("name",this.name).insertBefore(this.$input),this.events()}Ae=this}events(){this.$input.on("input",this.search.bind(this)),this.$input.on("keydown",this.preventEnterOnSearch.bind(this)),this.$input.on("keyup",this.listKeyEvents.bind(this))}setValue(e){this.selected=e||[],this.$hidden.val(JSON.stringify(this.selected)),this.populateTarget()}search(e){let t=E.trim(E(e.target).val());t.length>2?(this.ajax&&this.ajax.abort(),this.timeout&&clearTimeout(this.timeout),this.listInit(e),this.timeout=setTimeout(s=>{this.ajax=E.post(be.ajax_url,{_ajax_nonce:be.nonce,action:"acex_search_product",search:t},function(a){Ae.result=a.data,Ae.resultList(),console.log("result",a.data)})},400,e)):this.$list.length&&(this.$list.remove(),this.$list=E())}listInit(e){console.log("listInit",this.$list),!(this.$list&&this.$list.length)&&(this.$list=E('<div class="acex-results">').html(be.SVG.loader).insertAfter(e.target),this.$list.css({position:"absolute",zIndex:9999,width:this.$input.outerWidth(),top:this.$input.offset().top+this.$input.outerHeight(),left:this.$input.offset().left}).appendTo("body"),this.updateListPosition=()=>{this.$list.length&&this.$list.css({top:this.$input.offset().top+this.$input.outerHeight(),left:this.$input.offset().left,width:this.$input.width()})},E(window).off("scroll.acex-search resize.acex-search"),E(window).on("scroll.acex-search resize.acex-search",this.updateListPosition))}resultList(){this.$list.empty();let e=this.selected.map(t=>t[0]);if(this.result=this.result.filter(t=>!e.includes(String(t.ID))),!this.result.length){this.$list.remove(),this.$list=E();return}this.result.forEach(t=>{E('<div class="acex-row">').attr("data-id",t.ID).attr("data-title",t.post_title).attr("data-sku",t.sku).html(t.sku+" - "+t.post_title).appendTo(this.$list).click(this.listSelect.bind(this))}),this.listCurrent=0,this.listActive(),E("body").off("click.acex-search"),E("body").on("click.acex-search",t=>{E(t.target).closest(".acex-results").length||this.$list?.length&&(this.$list.remove(),this.$list=E())})}listKeyEvents(e){this.$list.length&&(this.$list.children().length<=1||(e.keyCode==38&&(this.listCurrent--,this.listCurrent<0&&(this.listCurrent=this.$list.children().length),this.listActive()),e.keyCode==40&&(this.listCurrent++,this.listCurrent>this.$list.children().length&&(this.listCurrent=0),this.listActive()),e.keyCode==13&&(e.preventDefault(),e.stopPropagation(),this.listSelect())))}listActive(){this.$list.children().removeClass("actus-active"),this.$list.children().eq(this.listCurrent).addClass("actus-active")}listSelect(e){let t=this.$list.children().eq(this.listCurrent);e&&(t=E(e.target));let s=t.attr("data-id"),a=t.attr("data-sku"),i=t.attr("data-title");this.$list.remove(),this.$list=E(),this.selected.push([s,t.text()]),this.$hidden.val(JSON.stringify(this.selected)),this.onChange(null,this.selected),this.populateTarget()}onChange(e,t){e&&(this.callback=e),t&&this.callback(t)}removeItem(e){let t=E(e.target).closest(".acex-row"),s=t.attr("data-id");t.remove(),this.selected.forEach((a,i)=>{a[0]==s&&this.selected.splice(i,1)}),this.$hidden.val(JSON.stringify(this.selected)),this.onChange(null,this.selected),this.populateTarget()}preventEnterOnSearch(e){e.keyCode==13&&(e.preventDefault(),e.stopPropagation())}populateTarget(){this.$target?.length&&(this.$target.empty(),this.selected.forEach(e=>{let t=E('<div class="acex-row actus-tag">').attr("data-id",e[0]).html(e[1]).appendTo(this.$target);E(be.SVG.times).appendTo(t).click(this.removeItem.bind(this))}))}},ct=Ee;var C=window.jQuery||jQuery,dt=window.acex_phpDATA||{},ss=dt.strings||{};function De(d,e){let t=C(`
	...
	`);e.append(t);let s=new ct(e.find(".acex-search-products"),C(".acex-specific-products-target"));s.setValue(d.specific_products||""),s.onChange(a=>{console.log("Specific Products changed:",a),d.specific_products=a,C("body").trigger("acex::filter-changed-query",[d,C()])}),C("body").off("change.acex-select-products",".acex-product-filters .acex-status-checkbox"),C("body").on("change.acex-select-products",".acex-product-filters .acex-status-checkbox",a=>{let i=C(a.currentTarget),o=i.val();C("body").trigger("acex::filter-changed",["status",o,i])}),C("body").off("change.acex-select-products",".acex-product-filters .acex-type-checkbox"),C("body").on("change.acex-select-products",".acex-product-filters .acex-type-checkbox",a=>{let i=C(a.currentTarget),o=i.val();C("body").trigger("acex::filter-changed",["product_type",o,i])}),C("body").off("change.acex-select-products",'.acex-product-filters input[type="date"]'),C("body").on("change.acex-select-products",'.acex-product-filters input[type="date"]',a=>{let i=C(a.currentTarget),o=i.val(),r=i.attr("id");C("body").trigger("acex::filter-changed",[r,o,i])}),C("body").off("click.acex-select-products",".acex-product-filters .actus-btn-secondary"),C("body").on("click.acex-select-products",".acex-product-filters .actus-btn-secondary",a=>{a.preventDefault();let i=C(a.currentTarget),o,r,n=i.siblings(".actus-label").text().toLowerCase();n.includes("created")?(o="date_from",r="date_to"):n.includes("modified")&&(o="modified_from",r="modified_to"),C("body").trigger("acex::filter-clear-date",[o,r,i])}),C("body").off("change.acex-select-products",'.acex-product-filters input[type="text"]'),C("body").on("change.acex-select-products",'.acex-product-filters input[type="text"]',a=>{let i=C(a.currentTarget),o=i.val(),r=i.attr("id");C("body").trigger("acex::filter-changed",[r,o,i])})}var ue=window.jQuery||jQuery,ut=window.acex_phpDATA||{},is=ut.strings||{};function Pe(d,e,t){let s=(n,c=0,l=new Set)=>{let u=[];for(let h of n){if(l.has(h.term_id))continue;let p=h.parent!==void 0?h.parent:h.category_parent;if(p==c){l.add(h.term_id);let m={id:h.term_id,name:h.name,slug:h.slug,parent:p,count:h.count||0,children:[]};m.children=s(n,h.term_id,l),u.push(m)}}return u},a=n=>n.length?`<ul>
	...
	`);e.append(a),pe("body").off("click.acex-select-products",".acex-filter-tags .actus-tag"),pe("body").on("click.acex-select-products",".acex-filter-tags .actus-tag",i=>{let o=pe(i.currentTarget),r=decodeURIComponent(o.attr("data-slug"));pe("body").trigger("acex::filter-changed",["tags",r,o])})}var he=window.jQuery||jQuery,Fe=window.acex_phpDATA||{},cs=Fe.strings||{};async function Oe(d,e,t){let s="",a=!1;await j("attributes"),t=Fe.product_attributes||[],t.forEach(o=>{let r="";if(o.terms&&o.terms.length){let n=o.attribute_name;r=`
		...
		<div class="acex-filter acex-premium ${a?"actus-open":""}">
			<h3>Attributes<span>${Fe.SVG.caret}</span></h3>
			<div class="acex-filter-body">
			...
			<div class="acex-filter-body">
				...
				<div class="acex-filter-row acex-filter-low-stock-quantity actus-flex">
					...
					<div class="acex-filter-classes">
						${Le.product_shipping_classes.map(a=>`<span class="actus-tag ${d.shipping_class?.includes(String(a.slug))?"actus-selected":""}" data-id="${a.term_id}" data-slug="${a.slug}">${a.name}</span>`).join("")}
					</div>
				</div>

			</div>
		</div>
	`);e.append(s),O("body").off("click.acex-select-products",".acex-stock-manage span"),O("body").on("click.acex-select-products",".acex-stock-manage span",a=>{let i=O(a.currentTarget);i.siblings().removeClass("actus-selected"),i.addClass("actus-selected");let o=i.attr("alt");o==="clear"?delete d.manage_stock:d.manage_stock=o,O("body").trigger("acex::filter-changed-query",[d,i])}),O("body").off("change.acex-select-products",".acex-stock-status-checkbox"),O("body").on("change.acex-select-products",".acex-stock-status-checkbox",a=>{let i=O(a.currentTarget),o=i.val();O("body").trigger("acex::filter-changed",["stock_status",o,i])}),O("body").off("change.acex-select-products",".acex-filter-stock-quantity input"),O("body").on("change.acex-select-products",".acex-filter-stock-quantity input",a=>{let i=O(a.currentTarget),o=parseInt(i.val()),r=i.attr("id");O("body").trigger("acex::filter-changed",[r,o,i])}),O("body").off("click.acex-select-products",".acex-filter-classes .actus-tag"),O("body").on("click.acex-select-products",".acex-filter-classes .actus-tag",a=>{let i=O(a.currentTarget),o=i.attr("data-slug");O("body").trigger("acex::filter-changed",["shipping_class",o,i])})}var v=window.jQuery||jQuery,w=window.acex_phpDATA||{},Ts=w.strings||{},{__:It}=wp.i18n,Ft={query_index:0,query_name:"All Published Products",status:["publish"]};(!w.queries||!w.queries.length)&&(w.queries=Se.map(d=>({...d})));var Re=class{constructor(){this.categories=w.product_categories||[],this.tags=w.product_tags||[],this.attributes=w.product_attributes||[],this.shipping_classes=w.product_shipping_classes||[],this.count=0,this.products=[],this.current=w.prefs?.currentQuery||0,this.queries=w.queries||[],this.queries.length||(this.queries=predefinedQueries),this.query=JSON.parse(JSON.stringify(this.queries[this.current])),this.productCountTimeout=null,this.productsTimeout=null,this.template={columns:[]},this.$preview=v(".acex-products-preview"),this.queryUnsaved=!1,this.free=A.free}getProducts(){return console.log("SelectProducts.getProducts",this.products),this.products}async render(e,t){this.cleanup(),this.check=await A.check(),this.current=w.prefs?.currentQuery||0,this.current>=this.queries.length&&(this.current=0),this.query=JSON.parse(JSON.stringify(this.queries[this.current]||{})),this.target=e,this.template=t||{columns:[]},this.$status=v(`
		...
		`);e.append(s),this.$filtersContainer=s.find(".acex-select-filters"),this.$count=this.$status.find(".acex-selected-count"),this.$variable=this.$status.find(".acex-variable-status"),this.$preview=this.target.find(".acex-products-preview"),setTimeout(()=>{this.$preview=this.target.find(".acex-products-preview")},10),this.renderFilters(),v("body").off("acex::template-changed.acex-select-products"),v("body").on("acex::template-changed.acex-select-products",(a,i)=>{this.template=i})}te;async renderFilters(){await j("categories"),await j("tags"),await j("attributes"),await j("shipping_classes"),this.categories=w.product_categories||[],this.tags=w.product_tags||[],this.attributes=w.product_attributes||[],this.shipping_classes=w.product_shipping_classes||[],this.$filtersContainer.empty(),this.renderQueries(),De(this.query,this.$filtersContainer),Pe(this.query,this.$filtersContainer,this.categories),Ie(this.query,this.$filtersContainer,this.tags),Oe(this.query,this.$filtersContainer,this.attributes),je(this.query,this.$filtersContainer),He(this.query,this.$filtersContainer),this.filterEvents(),v("body").off("click.acex-select-products",".acex-filter h3"),v("body").on("click.acex-select-products",".acex-filter h3",function(){v(this).parent().toggleClass("actus-open")})}filterEvents(){v("body").off("acex::filter-changed-query.acex-select-products"),v("body").on("acex::filter-changed-query.acex-select-products",(e,t,s)=>{this.query=t,this.queryView()}),v("body").off("acex::filter-changed.acex-select-products"),v("body").on("acex::filter-changed.acex-select-products",(e,t,s,a)=>{if(a&&(a.prop("type")==="checkbox"||a.prop("type")==="radio")){a.is(":checked")?(this.query[t]=this.query[t]||[],this.query[t].push(s)):this.query[t]=this.query[t].filter(i=>i!==s),this.query[t]&&this.query[t].length==0&&delete this.query[t],this.queryView();return}if(a&&a.hasClass("actus-tag")){a.hasClass("actus-selected")?(a.removeClass("actus-selected"),this.query[t]=this.query[t].filter(i=>i!==s)):(a.addClass("actus-selected"),this.query[t]=this.query[t]||[],this.query[t].push(s)),this.query[t]&&this.query[t].length==0&&delete this.query[t],this.queryView();return}if(a&&a.is('input[type="number"]')){let i=a.val(),o=i===""?null:Number(i);o===null||isNaN(o)?delete this.query[t]:this.query[t]=o,this.queryView();return}s?this.query[t]=s:delete this.query[t],this.queryView()}),v("body").off("acex::filter-clear-date.acex-select-products"),v("body").on("acex::filter-clear-date.acex-select-products",(e,t,s,a)=>{a.closest(".acex-filter-row").find('input[type="date"]').val(""),delete this.query[t],delete this.query[s],this.queryView()})}renderQueries(){let e=v(".acex-query-group");e.length||(e=v(`
			...
			`),this.$filtersContainer.append(e)),e.find(".acex-query-view").empty();let t=e.find(".acex-queries-list").empty(),s=e.find(".acex-queries-buttons").empty();this.queryView(),this.queryList(t),v("body").off("click.acex-select-products",".acex-button-reset-queries"),v("body").on("click.acex-select-products",".acex-button-reset-queries",r=>{r.preventDefault(),this.resetQueries(t)}),v('<button class="actus-btn actus-btn-danger">').text("Delete Query").appendTo(s).on("click",async r=>{r.preventDefault(),await x.confirm("Are you sure you want to delete this query?")&&typeof this.deleteQuery=="function"&&(v(r.target).prop("disabled",!0).html(w.SVG.loader),await this.deleteQuery(this.current),v(r.target).prop("disabled",!1).text("Delete Query"))}),v('<button class="actus-btn actus-btn-secondary">').text("Add Query").appendTo(s).on("click",r=>{r.preventDefault(),this.addQuery()}),v('<button class="actus-btn actus-btn-primary acex-save-query">').text("Save Query").appendTo(s).on("click",async r=>{r.preventDefault(),this.queries[this.current]=JSON.parse(JSON.stringify(this.query)),this.queryUnsaved=!1,v(r.target).prop("disabled",!0).html(w.SVG.loader),await B("acex_queries",this.queries),v(r.target).prop("disabled",!1).text("Save Query").hide(),console.log("Query saved:",this.query)})}async resetQueries(e){await x.confirm("Are you sure you want to restore the default queries?<br>It will not affect your custom queries.")&&(this.queries=this.queries.filter(s=>!s._system),this.queries=[...Se,...this.queries],w.queries=this.queries,await B("acex_queries",w.queries),x.success("Default queries restored successfully."),this.queryList(e),setTimeout(()=>{e.find(".acex-query-item").first().trigger("click")},0))}queryList(e){e.empty(),this.queries.forEach((t,s)=>{let a=s===this.current?"actus-selected":"",i=t.p?"acex-premium":"",o=v(`
				<div class="acex-query-item ${a} ${i}" data-index="${s}">
					<span class="acex-query-name">${t.query_name}</span>
					</div>
				`);e.append(o)}),v("body").off("click.acex-query-list",".acex-query-item"),v("body").on("click.acex-query-list",".acex-query-item",async t=>{if(!this.check&&v(t.currentTarget).hasClass("acex-premium")||this.queryUnsaved&&!await x.confirm(It("You have unsaved changes to the current query. Are you sure you want to switch queries without saving?","actus-excel-for-woo")))return;this.queryUnsaved=!1;let s=v(t.currentTarget).attr("data-index"),a=v(t.currentTarget).index();typeof this.useQuery=="function"&&this.useQuery(s)})}useQuery(e){e=parseInt(e),this.current=e,this.query=JSON.parse(JSON.stringify(this.queries[this.current])),w.prefs.currentQuery=this.current,w.prefs.save(),v(".acex-query-item").removeClass("actus-selected"),v(`.acex-query-item[data-index="${e}"]`).addClass("actus-selected"),this.renderFilters()}async addQuery(){let e=await x.prompt("What is the name of the query?","Default Query","User Input");if(!e)return;let t={query_name:e};this.queries.push(t),this.current=this.queries.length-1,this.query=JSON.parse(JSON.stringify(t)),await B("acex_queries",this.queries),this.renderQueries()}async deleteQuery(e){this.queries.length<=1||(this.queries.splice(e,1),this.current>=this.queries.length&&(this.current=this.queries.length-1),this.current<0&&(this.current=0),this.query=JSON.parse(JSON.stringify(this.queries[this.current]))||Ft,await B("acex_queries",this.queries),this.renderQueries(),this.renderFilters())}queryView(){this.queryUnsaved=!1,v(".acex-save-query").hide(),JSON.stringify(this.query)!=JSON.stringify(this.queries[this.current])&&(this.queryUnsaved=!0,v(".acex-save-query").show());let e=v(".acex-query-group");e.find(".acex-query-view").empty(),this.getProductCount(),this.query.description=this.query.description||this.query.query_name||"";let t=Object.keys(this.query);t.includes("description")&&(t=["description",...t.filter(s=>s!=="description")]),t.forEach(s=>{if(s.startsWith("_")||s.startsWith("query_")||s.startsWith("template"))return;let a=JSON.stringify(this.query[s],null,2);s==="specific_products"&&Array.isArray(this.query.specific_products)&&this.query.specific_products.length&&(a=this.query.specific_products.map(r=>r[1].split(" - ")[1]||r[1]).join("<br>")),s==="tags"&&Array.isArray(this.query.tags)&&(a=this.query.tags.map(r=>{this.tags=w.product_tags||[];let n=this.tags.find(c=>String(c.slug)===String(r));return n?n.name:r}).join(", ")),s==="categories"&&Array.isArray(this.query.categories)&&(a=this.query.categories.map(r=>{this.categories=w.product_categories||[];let n=this.categories.find(c=>String(c.slug)===String(r));return n?n.name:r}).join(", ")),s==="shipping_class"&&Array.isArray(this.query.shipping_class)&&(a=this.query.shipping_class.map(r=>{this.shipping_classes=w.product_shipping_classes||[];let n=this.shipping_classes.find(c=>String(c.slug)===String(r));return n?n.name:r}).join(", ")),s==="attributes"&&typeof this.query.attributes=="object"&&(a=Object.entries(this.query.attributes).map(([r,n])=>{this.attributes=w.product_attributes||[],r=r.toLowerCase();let c=this.attributes.find(h=>h.attribute_name===r),l=c?c.attribute_label:r,u=Array.isArray(n)?n.map(h=>{let p=c&&c.terms?c.terms.find(m=>String(m.slug)===String(h)):null;return p?p.name:h}).join(", "):"";return`${l}: ${u}`}).join("<br>")),typeof a=="string"?a=a.replace(/^\[|\]$/g,"").replace(/^\{|\}$/g,""):Array.isArray(this.query[s])?a=a.replace(/^\[|\]$/g,""):typeof this.query[s]=="object"&&this.query[s]!==null&&(a=a.replace(/^\{|\}$/g,"")),a=a.trim(),a=a.replace(/^"|"$/g,"");let i=w.data_props[s]?w.data_props[s].label:s,o=v(`
				<div class="acex-query-field" data-key="${s}">
					<label>${i}</label>
					<div class="acex-query-value" data-key="${s}" data-value="${this.query[s]||""}">${a}</div>
				</div>
			`);if(e.find(".acex-query-view").append(o),s==="specific_products")return e.find(".acex-query-view").html(o),!1})}cleanup(){this.productCountTimeout&&(clearTimeout(this.productCountTimeout),this.productCountTimeout=null),this.productsTimeout&&(clearTimeout(this.productsTimeout),this.productsTimeout=null),v("body").off(".selectProducts"),v("body").off(".acex-props"),this.$preview=null,this.$filtersContainer=null,this.$status=null,this.$count=null,this.$variable=null}async getProductCount(){if(this.$count.text(0),!!this.query&&Object.keys(this.query).length)return this.productCountTimeout&&(clearTimeout(this.productCountTimeout),this.productCountTimeout=null),new Promise(e=>{this.$count.html(w.SVG.loader),this.$preview.html(`<div class="actus-loader">${w.SVG.loader}</div>`),this.productCountTimeout=setTimeout(async()=>{let t=JSON.parse(JSON.stringify(this.query));t.specific_products&&(t.ids=t.ids||[],t.ids=[...new Set([...t.ids,...t.specific_products])],t.ids=t.ids.join(","),delete t.specific_products),this.$count.html(w.SVG.loader),this.count=await ne(t,!0),this.$count.text(this.count||0),this.productCountTimeout=null,e(this.count),await this.fetchProducts(t)},1e3)})}async fetchProducts(e){if(e&&Object.keys(e).length)return this.productsTimeout&&(clearTimeout(this.productsTimeout),this.productsTimeout=null),new Promise(t=>{this.productsTimeout=setTimeout(async()=>{let s=await ne(e);this.products=s.products||[],this.productsTimeout=null,t(this.products);let a=this.products.slice(0,20);G.setContainer(this.$preview),G.create(this.template,a)},1e3)})}},le=new Re;var Ds=window.jQuery||jQuery,oe=window.acex_phpDATA||{},Ps=oe.strings||{},Ne=class{constructor(e,t){this.template=e||{},this.createSheet=t}async addValidations(e,t,s){let a=[],i=0,o=e.addWorksheet("_acex_helpers");o.state="hidden";let r=0,n=[],c=[];for(let l of this.template.columns){if(Object.keys(oe.data_taxonomies).includes(l.key)){let u=await this.validation_taxonomies(e,t,s,l.key,i,o,r);e=u.workbook,t=u.worksheet,i=u.insertedColumnsCount,r=u.helperSheetColCount,u.conditionalFormattingInfo&&n.push(u.conditionalFormattingInfo),u.formulaInfo&&c.push(u.formulaInfo)}if(l.key==="shipping_class"&&a.push({column:l.key,referenceSheet:"Shipping Classes",referenceHeader:"Slug",referenceColumn:"B",startRow:3}),Array.isArray(l.type)||typeof l.type=="string"&&l.type.includes(",")){let u=Array.isArray(l.type)?l.type:l.type.split(",").map(h=>h.trim());a.push({column:l.key,options:u})}if(l.type==="bool"||l.type==="boolean"){a.push({column:l.key,options:["true","false"]});let u=this.template.columns.findIndex(m=>m.key===l.key),h=this.getExcelColumnLetter(u+1),p=s+2+1e3;t.addConditionalFormatting({ref:`${h}3:${h}${p}`,rules:[{type:"expression",formulae:[`LOWER(${h}3)="true"`],style:{fill:{type:"pattern",pattern:"solid",bgColor:{argb:"FFD4EDDA"}},font:{color:{argb:"FF155724"},bold:!0}}},{type:"expression",formulae:[`LOWER(${h}3)="false"`],style:{fill:{type:"pattern",pattern:"solid",bgColor:{argb:"FFF8D7DA"}},font:{color:{argb:"FF721C24"}}}}]}),t.getColumn(u+1).alignment={horizontal:"center",vertical:"middle"}}}c.forEach(l=>{this.applyTaxonomyFormulas(t,l)}),n.forEach(l=>{let u=this.getExcelColumnLetter(l.colIndex),h=this.getExcelColumnLetter(l.helperColIndex);t.addConditionalFormatting({ref:`${u}3:${u}${l.maxRow}`,rules:[{type:"expression",formulae:[`INDIRECT("'${l.helperSheetName}'!${h}"&ROW())>0`],style:{fill:{type:"pattern",pattern:"solid",bgColor:{argb:"FFFF9999"}}}}]})}),this.validation_dropdowns(a,t,s)}validation_dropdowns(e,t,s){e.length&&e.forEach(a=>{let{column:i,options:o,referenceSheet:r,referenceHeader:n,referenceColumn:c,startRow:l}=a,u=this.template.columns.findIndex(g=>g.key===i);if(u===-1)return;let h=0;for(let g=0;g<u;g++){let f=this.template.columns[g];Object.keys(oe.data_taxonomies).includes(f.key)&&h++}let p=u+h,m=this.getExcelColumnLetter(p+1);if(r&&(!o||!o.length)){let g=c||"B",f=l||3,y=t.workbook.getWorksheet(r),T=f;if(y){let L=y.actualRowCount||y.rowCount||f;for(let W=L;W>=f;W--){let de=y.getCell(`${g}${W}`).value;if(de!=null&&de!==""){T=W;break}}}let S=`'${r}'!$${g}$${f}:$${g}$${T}`;for(let L=3;L<=s+2;L++){let W=t.getCell(`${m}${L}`);W.dataValidation={type:"list",allowBlank:!0,formulae:[S],showErrorMessage:!0,errorTitle:"Invalid Selection",error:`Please select a value from the ${n||"Options"} list`}}return}if(r&&o&&o.length>0){let g=c||"B",f=l||2,y=`'${r}'!$${g}$${f}:$${g}$${o.length+f-1}`;for(let T=3;T<=s+2;T++){let S=t.getCell(`${m}${T}`);S.dataValidation={type:"list",allowBlank:!0,formulae:[y],showErrorMessage:!0,errorTitle:"Invalid Selection",error:`Please select a value from the ${n||"Options"} list`}}}else if(o&&o.length>0){let g=`"${o.join(",")}"`;if(g.length<=255)for(let f=3;f<=s+2;f++){let y=t.getCell(`${m}${f}`);y.dataValidation={type:"list",allowBlank:!0,formulae:[g],showErrorMessage:!0,errorTitle:"Invalid Selection",error:"Please select a value from the dropdown list"}}else{let f=`${i}_options`,y=workbook.addWorksheet(f);y.addRow(["Options"]),y.getRow(1).font={bold:!0},o.forEach(S=>{y.addRow([S])});let T=`${f}!$A$2:$A$${o.length+1}`;for(let S=3;S<=s+2;S++){let L=t.getCell(`${m}${S}`);L.dataValidation={type:"list",allowBlank:!0,formulae:[T],showErrorMessage:!0,errorTitle:"Invalid Selection",error:"Please select a value from the Options list"}}}}})}async validation_taxonomies(e,t,s,a="categories",i=0,o,r=0){let n={};for(let u in oe.data_taxonomies){await j(u);let h=oe.data_taxonomies[u],p=h.labels.menu_name||h.label||u;n[u]={sheetName:p,columnKey:u,dataSource:oe[`product_${u}`],headers:["id","slug","name","description"],hasParent:!1,comment:"Multiple tags: Select from dropdown, then press F2 to edit. Add more tags on new lines (Alt+Enter) or comma-separated.",displayColumnName:p,helperColumnPrefix:u+" Name Helper",validationHelperTitle:p+" Validation (Helper)",validationRange:`${p}!$B:$B`,nameRange:`${p}!$B:$C`,promptTitle:"Enter Slugs",promptText:`Enter taxonomy slugs separated by:
...
\u{1F4D6} See ${p} sheet for the Slug/Name list.`,useIds:!1,vlookupColumn:2,lookupColumn:1,namesWidth:25},h.hierarchical&&(n[u].headers=["id","slug","name","description","parent","parent name","parent slug"],n[u].hasParent=!0)}let c=n[a];if(!c)return console.error(`Unknown taxonomy type: ${a}`),{workbook:e,worksheet:t};let l=this.createSheet(c.sheetName,c.headers,c.comment);if(c.dataSource&&c.dataSource.length>0){c.dataSource.forEach(p=>{let m=[p.term_id,this.decodeHtmlEntities(p.slug),this.decodeHtmlEntities(p.name),p.description];if(c.hasParent){let g=c.dataSource.find(f=>f.term_id===p.parent);m.push(p.parent),m.push(g?this.decodeHtmlEntities(g.name):""),m.push(g?this.decodeHtmlEntities(g.slug):"")}l.addRow(m)});let u=c.hasParent?[10,40,40,60,20,40]:[10,40,40,60];u.forEach((p,m)=>{l.getColumn(m+1).width=p}),l.getColumn(1).alignment={horizontal:"center",vertical:"top"};for(let p=2;p<=u.length;p++)l.getColumn(p).alignment={horizontal:"left",vertical:"top",indent:1};l.getColumn(u.length).alignment={horizontal:"left",vertical:"top",indent:1,wrapText:!0};let h=this.addTaxonomyValidation(e,t,s,c,i,o,r);if(h)return h}return{workbook:e,worksheet:t,insertedColumnsCount:i,helperSheetColCount:r}}addTaxonomyValidation(e,t,s,a,i,o,r){let n=this.template.columns.findIndex(c=>c.key===a.columnKey);if(n!==-1){let c=0;for(let z=0;z<n;z++){let se=this.template.columns[z];Object.keys(oe.data_taxonomies).includes(se.key)&&c++}let l=n+c,u=l+2;t.spliceColumns(u,0,[]);let h=this.getExcelColumnLetter(l+1),p=this.getExcelColumnLetter(u);t.getColumn(u).width=a.namesWidth,t.getCell(`${p}1`).value=a.displayColumnName,t.getCell(`${p}1`).font={bold:!0,color:{argb:"FFFFFFFF"}},t.getCell(`${p}1`).alignment={horizontal:"center"},t.getCell(`${p}1`).fill={type:"pattern",pattern:"solid",fgColor:{argb:"FF777777"}},t.getCell(`${p}2`).value="Auto-filled",t.getCell(`${p}2`).alignment={wrapText:!0,vertical:"middle",horizontal:"center"},t.getCell(`${p}2`).font={italic:!0,color:{argb:"FF888888"}};let m=u,g=t.lastRow?.number||2,y=Math.min(Math.max(g,s+2)+100,1e4),T=this._addTaxonomyHelpers(o,a,y,r,t.name,h);for(let z=3;z<=y;z++){let se=t.getCell(`${h}${z}`);se.dataValidation={type:"textLength",operator:"lessThan",formulae:[1e4],allowBlank:!0,showInputMessage:!0,promptTitle:a.promptTitle,prompt:a.promptText,showErrorMessage:!1}}let S={colIndex:l+1,helperColIndex:T.validationHelperColIndex,helperSheetName:o.name,maxRow:y},L={cfg:a,maxRow:y,worksheetName:t.name,colIndex:l+1,displayColIndex:m,helperSheetName:o.name,nameHelperStartIndex:T.nameHelperStartIndex,nameHelperCount:T.nameHelperCount,validationHelperColIndex:T.validationHelperColIndex},W=t.getCell(1,l+1);W.note={texts:[{font:{size:11,color:{argb:"FF000000"}},text:a.headerNote}]};let de=t.getColumn(l+1),$e=this.estimateColumnWidth(this.template.columns[n].header),St=Math.min(t.rowCount||s+10,s+10);for(let z=3;z<=St;z++){let se=t.getCell(z,l+1);if(se&&se.value){let At=this.estimateColumnWidth(String(se.value||""));$e=Math.max($e,At)}}return de.width=Math.min(Math.max($e,10)+2,50),i+=1,{workbook:e,worksheet:t,insertedColumnsCount:i,helperSheetColCount:T.newHelperSheetColCount,conditionalFormattingInfo:S,formulaInfo:L}}return!1}_addTaxonomyHelpers(e,t,s,a,i,o){let n=a+1;for(let u=0;u<10;u++){let h=n+u,p=this.getExcelColumnLetter(h);e.getColumn(h).width=30,e.getCell(`${p}1`).value=`${t.helperColumnPrefix} ${u+1}`,e.getCell(`${p}1`).font={italic:!0,color:{argb:"FFBBBBBB"}};for(let m=3;m<=s;m++){let g=(u+1)*99-98,f=`LET(_raw,SUBSTITUTE('${i}'!${o}${m},CHAR(10),","),_token,TRIM(MID(SUBSTITUTE(_raw,",",REPT(" ",99)),${g},99)),IF(_token="","",IFERROR(VLOOKUP(_token,${t.nameRange},${t.vlookupColumn},FALSE),"[invalid slug]")))`;e.getCell(`${p}${m}`).value={formula:f}}}let c=n+10,l=this.getExcelColumnLetter(c);e.getColumn(c).width=15,e.getCell(`${l}1`).value=t.validationHelperTitle,e.getCell(`${l}1`).font={bold:!0,color:{argb:"FF888888"}},e.getCell(`${l}2`).value=`0 = Valid IDs
>0 = Invalid`,e.getCell(`${l}2`).alignment={wrapText:!0,vertical:"top"},e.getCell(`${l}2`).font={italic:!0,color:{argb:"FF888888"}};for(let u=3;u<=s;u++){let h=`IF('${i}'!${o}${u}="",0,LET(_raw,SUBSTITUTE('${i}'!${o}${u},CHAR(10),","),_arr,IFERROR(TEXTSPLIT(_raw,","),TRIM(MID(SUBSTITUTE(_raw,",",REPT(" ",99)),ROW(INDIRECT("1:200"))*99-98,99))),_trim,TRIM(_arr),SUMPRODUCT(--(LEN(_trim)>0),--(COUNTIF(${t.validationRange},_trim)=0))))`;e.getCell(`${l}${u}`).value={formula:h}}return{nameHelperStartIndex:n,nameHelperCount:10,validationHelperColIndex:c,newHelperSheetColCount:a+10+1}}applyTaxonomyFormulas(e,t){let{cfg:s,maxRow:a,colIndex:i,displayColIndex:o,helperSheetName:r,nameHelperStartIndex:n,nameHelperCount:c,validationHelperColIndex:l}=t,u=this.getExcelColumnLetter(i),h=this.getExcelColumnLetter(o),p=[];for(let f=0;f<c;f++)p.push(this.getExcelColumnLetter(n+f));let m=p[0],g=p[p.length-1];for(let f=3;f<=a;f++){let y=e.getCell(`${h}${f}`),T=`IF(${u}${f}="", "", LET(_names,'${r}'!${m}${f}:${g}${f},_joined,TEXTJOIN(CHAR(10),TRUE,_names),IF(_joined="","[no matches]",_joined)))`,S=e.getRow(f);S.height=void 0,y.value={formula:T},y.alignment={wrapText:!0,vertical:"top",horizontal:"center"},y.fill={type:"pattern",pattern:"solid",fgColor:{argb:"FFEFF5FF"}},y.font={color:{argb:"FF1F4E78"}},y.protection={locked:!0},y.border={top:{style:"thin",color:{argb:"FFDDDDDD"}},bottom:{style:"thin",color:{argb:"FFDDDDDD"}}}}}getExcelColumnLetter(e){let t="";for(;e>0;){let s=(e-1)%26;t=String.fromCharCode(65+s)+t,e=Math.floor((e-1)/26)}return t}decodeHtmlEntities(e){if(!e||typeof e!="string")return e;try{return decodeURIComponent(e)}catch{return e}let t=document.createElement("textarea");return t.innerHTML=e,t.value}estimateColumnWidth(e){if(!e)return 8;let s=String(e).split(`
`).reduce((i,o)=>o.length>i.length?o:i,""),a=0;for(let i of s)/[A-Z0-9@#$%&*]/.test(i)?a+=1.2:/[il!|']/.test(i)?a+=.5:a+=1;return Math.ceil(a)}},Be=Ne;var Ls=window.jQuery||jQuery,D=window.acex_phpDATA||{},Hs=D.strings||{},{__:Rs}=wp.i18n,Me=class{constructor(e,t){this.template=e||{},this.query=t||{},this.free=A.free,this.workbook=null,this.worksheet=null}async create2(e){let t=await this.createBase(),s=await this.createWorksheet();this.addProductRows(e);let a=await this.finalize(e)}async createBase(){if(typeof ExcelJS>"u")throw new Error('ExcelJS library is not loaded. Please include: <script src="https://cdn.jsdelivr.net/npm/exceljs@4.3.0/dist/exceljs.min.js"><\/script>');return this.check=await A.check(),this.KEYS=this.template.columns,this.check||(this.KEYS=this.template.columns.filter(e=>this.free.props.includes(e.key))),this.workbook=new ExcelJS.Workbook,this.workbook.creator="Actus Excel for WooCommerce",this.workbook.created=new Date,this.workbook.calcProperties.fullCalcOnLoad=!0,this.workbook}async createWorksheet(){let e=this.template.sheet||"Products";this.worksheet=this.workbook.addWorksheet(e),this.worksheet.properties.outlineLevelRow=0,this.worksheet.properties.defaultRowHeight=15,this.worksheet.properties.dyDescent=0,this.worksheet.properties.defaultColWidth=10;let t=this.createHeaders(this.worksheet);return this.worksheet=this.addDescriptions(this.worksheet),this.worksheet.views=[{state:"frozen",ySplit:1}],this.worksheet._headers=t,this.rows=["H","h"],this.worksheet}async finalize(){return this.autoSizeColumns(),this.styleCells(),await this.createShippingClassesSheet(),await this.createAttributesSheet(),await this.createAttributesTermsSheet(),await new Be(this.template,this.createSheet.bind(this)).addValidations(this.workbook,this.worksheet,this.rows.length-2),this.worksheet.protect("",{selectLockedCells:!0,selectUnlockedCells:!0}),this.enforceRowHeightLimit(150),this.workbook}async create(e){if(typeof ExcelJS>"u")throw new Error('ExcelJS library is not loaded. Please include: <script src="https://cdn.jsdelivr.net/npm/exceljs@4.3.0/dist/exceljs.min.js"><\/script>');this.check=await A.check(),this.KEYS=this.template.columns,this.check||(this.KEYS=this.template.columns.filter(r=>this.free.props.includes(r.key)));let t=new ExcelJS.Workbook;t.creator="Actus Excel for WooCommerce",t.created=new Date,t.calcProperties.fullCalcOnLoad=!0;let s=this.template.sheet||"Products",a=t.addWorksheet(s),i=this.createHeaders(a);return a=this.addDescriptions(a),a.views=[{state:"frozen",ySplit:2}],this.addProductRows(a,e),this.autoSizeColumns(a,e,i),this.styleCells(a,e),await this.createShippingClassesSheet(),await this.createAttributesSheet(),await this.createAttributesTermsSheet(),await new Be(this.template,this.createSheet.bind(this)).addValidations(t,a,e.length),a.protect("",{selectLockedCells:!0,selectUnlockedCells:!0}),t}createHeaders(e){let t=this.KEYS.map(a=>a.header);e.addRow(t),e.getRow(1).font={bold:!0,color:{argb:"FFFFFFFF"}},e.getRow(1).fill={type:"pattern",pattern:"solid",fgColor:{argb:"FF777777"}},e.getRow(1).alignment={horizontal:"center",vertical:"middle"};let s=this.KEYS.map(a=>a.key);return e.addRow(s),e.getRow(2).alignment={horizontal:"center",vertical:"middle"},t}addProductRows(e){for(let t of e){let s="-",a=t.type==="variation";a&&(s="variation"),this.rows.push(s);let i=this.KEYS.map(r=>{let n=this.getProductValue(t,r.key,r.type);if(r.key.includes("date_")&&n){let l=new Date(n);if(!isNaN(l.getTime()))return l}if(typeof n=="object"&&n!==null)return JSON.stringify(n);let c=D.data_mustBeInherited&&D.data_mustBeInherited.includes(r.key);return a&&c&&r.key,n??""}),o=this.worksheet.addRow(i);this.KEYS.forEach((r,n)=>{if(r.key.includes("date_")){let c=o.getCell(n+1);c.numFmt="yyyy-mm-dd hh:mm:ss"}})}}autoSizeColumns(){this.worksheet._headers.forEach((e,t)=>{let s=this.worksheet.getColumn(t+1),a=this.estimateColumnWidth(e),i=0,o=Math.min(this.rows.length,1e3);for(let r=3;r<=o;r++){let n=this.worksheet.getCell(r,t+1);if(n&&n.value){let c=this.estimateColumnWidth(String(n.value||""));i=Math.max(i,c)}}s.width=Math.min(Math.max(a,i,10)+2,50)})}styleCells(){let e=this.rows.length;for(let t=3;t<=e;t++){let s=this.worksheet.getRow(t),a=this.rows[t-1]==="variation",i=15;this.KEYS.forEach((n,c)=>{let l=s.getCell(c+1),h=n&&(n.type==="number"||n.type==="float"||n.type==="int"||n.type==="integer"||n.type==="decimal")?"right":"left";h=n.align||h,l.alignment={wrapText:!0,vertical:"top",horizontal:h,indent:1};let p=D.data_mustBeInherited&&D.data_mustBeInherited.includes(n.key);a&&p?(l.protection={locked:!0},l.fill={type:"pattern",pattern:"solid",fgColor:{argb:"FFE0E0E0"}},l.font={color:{argb:"FF999999"}}):l.protection={locked:!1};let m=this.calculateCellHeight(l.value,c,this.KEYS)||15;i=Math.max(i,m)});let r=Math.min(i,100);s.height=r,s.model.customHeight=!0,s.customHeight=!0,s.commit()}this.worksheet.getRow(2).alignment={horizontal:"center",vertical:"middle"}}addDescriptions(e){let t=this.KEYS.map(s=>s.key);return this.KEYS.forEach((s,a)=>{D.data_props[s.key]=D.data_props[s.key]||{};let i="";i=D.data_props&&D.data_props[s.key]&&D.data_props[s.key].description?D.data_props[s.key].description:"";let o="FFBBBBBB";D.data_warnings&&D.data_warnings[s.key]&&(o="FFFFB84D",i+=`

\u26A0\uFE0F `+D.data_warnings[s.key]),D.data_restrictions&&D.data_restrictions[s.key]&&(i+=`

\u2757 `+D.data_restrictions[s.key],o="FFFFAAAA");let r=e.getCell(2,a+1),n=i.split(`
`),c=[{font:{bold:!0,size:11,color:{argb:"FF000000"},name:"Calibri"},text:D.data_props[s.key].meta_key+`
`}];n.forEach((l,u)=>{c.push({font:{size:11,color:{argb:"FF000000"},name:"Calibri"},text:l+(u<n.length-1?`
`:"")})}),r.note={texts:c,margins:{insetmode:"auto"}},r.fill={type:"pattern",pattern:"solid",fgColor:{argb:o}},r.font={italic:!0,color:{argb:"FFFFFFFF"}},r.alignment={wrapText:!0,vertical:"top",horizontal:"center",indent:1}}),e}async createShippingClassesSheet(){if(!this.KEYS.some(i=>i.key==="shipping_class"))return;let t=[{header:"ID",key:"term_id",alignment:{horizontal:"center",indent:1}},{header:"Slug",key:"slug",alignment:{horizontal:"left",indent:1}},{header:"Name",key:"name",alignment:{horizontal:"left",indent:1}},{header:"Description",key:"description",alignment:{horizontal:"left",indent:1}},{header:"Count",key:"count",alignment:{horizontal:"center",indent:1}}],s=[];await j("shipping_classes"),(D.product_shipping_classes||[]).forEach(i=>{let o=i.term_id||"",r=i.slug||"",n=i.name||"",c=i.description||"",l=i.count||0;s.push([o,r,n,c,l])}),this.createSheet("Shipping Classes",t,"Product shipping classes configuration",s)}async createAttributesSheet(){let e=["attributes","attribute_values","default_attributes"];if(!this.KEYS.some(o=>e.includes(o.key)))return;let s=[{header:"ID",key:"id",alignment:{horizontal:"center",indent:1}},{header:"Slug",key:"slug",alignment:{horizontal:"left",indent:1}},{header:"Label",key:"label",alignment:{horizontal:"left",indent:1}},{header:"Type",key:"type",alignment:{horizontal:"left",indent:1}},{header:"Is Visible",key:"is_visible",alignment:{horizontal:"center",indent:1}},{header:"Options",key:"options",alignment:{horizontal:"left",wrapText:!0,indent:1}}],a=[];await j("attributes"),(D.product_attributes||[]).forEach(o=>{let r=o.attribute_id||"",n=o.attribute_name||"",c=o.attribute_label||"",l=o.attribute_type||"select",u=o.attribute_public===1?"Yes":"No",p=(o.terms||[]).map(m=>this.decodeHtmlEntities(m.slug)).join(`
`);a.push([r,n,c,l,u,p])}),this.createSheet("Attributes",s,"Product attributes configuration",a)}async createAttributesTermsSheet(){let e=["attributes","attribute_values","default_attributes"];if(!this.KEYS.some(o=>e.includes(o.key)))return;let s=[{header:"Attribute",key:"attribute_name",alignment:{horizontal:"center",indent:1}},{header:"Term ID",key:"term_id",alignment:{horizontal:"center",indent:1}},{header:"Name",key:"name",alignment:{horizontal:"left",indent:1}},{header:"Slug",key:"slug",alignment:{horizontal:"left",indent:1}},{header:"Description",key:"description",alignment:{horizontal:"left",indent:1}},{header:"Count",key:"count",alignment:{horizontal:"center",indent:1}}],a=[];await j("attributes"),(D.product_attributes||[]).forEach(o=>{let r=o.attribute_name||"";(o.terms||[]).forEach(c=>{let l=c.term_id||"",u=this.decodeHtmlEntities(c.name)||"",h=this.decodeHtmlEntities(c.slug)||"",p=this.decodeHtmlEntities(c.description)||"",m=c.count||0;a.push([r,l,u,h,p,m])})}),this.createSheet("Attribute Terms",s,"Detailed list of all attribute terms with their properties",a)}getProductValue(e,t,s){if(t.includes(".")){let i=t.split("."),o=e;for(let r of i)if(o&&typeof o=="object")o=o[r];else return null;return this.decodeHtmlEntities(o)}let a=e[t];if(a===void 0&&e.meta_data&&(a=e.meta_data[t]),a===void 0&&e.meta_data){let i=t.startsWith("_")?t:"_"+t;a=e.meta_data[i]}if(s==="bool"||s==="boolean")return a?"true":"false";if(s==="int"){let i=parseInt(a);return isNaN(i)?0:i}if(s==="float"){let i=parseFloat(a);return isNaN(i)?0:i}if(Object.keys(D.data_taxonomies).includes(t))return(Array.isArray(a)?a.map(o=>o&&o.slug!==void 0?this.decodeHtmlEntities(o.slug):this.decodeHtmlEntities(o)):[]).filter(o=>o!=null&&o!=="").join(`
...
<div class="google-auth-info">
	...
	<div class="privacy-note">
		...
		`),t.errors=i,t.warnings=o,t.HTML=s+a,t}startImportModal(e,t){let s=new x({title:"Import Product Data",className:"acex-import-modal"});if(!t.length)s.setContent("<p>No products found matching your filters.</p>"),s.updateButtons([{text:"Close",class:"actus-btn-secondary",onClick:a=>{a.destroy(),this.init()}}]);else{s.setContent(`
			...
			`),s.updateButtons([{text:"Close",class:"actus-btn-secondary",onClick:i=>i.destroy()},{text:"Start Import",class:"actus-btn-primary",onClick:i=>{this.importOptions.importAsNew=s.$body.find(".acex-field-checkbox input.acex-input").is(":checked")||"",this.importProducts(t,e,s)}}])}s.open()}idValidationHtml(e){let t="",s=e[0]?.ajax||e[1]?.ajax;return s&&(t+='<div class="acex-import-circles">',s.create&&s.create.length&&(t+=`<div class="acex-import-circle">
				...
				</div>`),t+="</div>"),t}async importProducts(e,t,s){this.importOptions.ajaxData=t[0]?.ajax||t[1]?.ajax||{},this.check||e.length>LIMIT&&(e=e.slice(0,LIMIT));let a={},i={};i.import_as_new=this.importOptions.importAsNew||"",i.create=this.importOptions.ajaxData?.create,i.update_ids=this.importOptions.ajaxData?.update_ids,i.update_skus=this.importOptions.ajaxData?.update_skus,i.invalid=this.importOptions.ajaxData?.invalid;for(let l in ee.data_taxonomies)i[`ignore_${l}`]=this.importOptions?.taxonomiesData?.[`missing_${l}`]?.length&&!this.importOptions[`create_${l}`];if(i.ignore_attributes=(this.importOptions?.attributesData?.missing_attributes?.length||this.importOptions?.attributesData?.missing_values?.length)&&!this.importOptions.create_attributes,this.check){let l=!1;this.importOptions.create_attributes&&(l=!0);for(let u in ee.data_taxonomies)if(this.importOptions[`create_${u}`]){l=!0;break}if(l){let u="";for(let p in ee.data_taxonomies){let m=ee.data_taxonomies[p];this.importOptions[`create_${p}`]&&(u+="Creating "+m.label)}this.importOptions.create_attributes&&(u?u+=" and Attributes":u+="Creating Attributes"),s.showLoading(u);for(let p in ee.data_taxonomies)a[p]=this.importOptions[`create_${p}`]?this.importOptions?.taxonomiesData?.[`missing_${p}`]:[];a.attributes=this.importOptions.create_attributes?this.importOptions?.attributesData?.missing_attributes:[],a.values=this.importOptions.create_attributes?this.importOptions?.attributesData?.missing_values:[],a={...a,...bt(this.workbook,this.importOptions)};let h=await ot(a)}}s.showLoading("Importing product data to WooCommerce..."),s.updateButtons([]),s.addContent(`<div class="acex-import-status">
			<div class="acex-progress-bar"><div class="acex-progress-bar-fill"></div></div>
			</div>`);let o=V(".acex-import-status"),r=await rt(e,{chunkSize:20,_data:i,onProgress:(l,u,h)=>{let p=Math.round(l/u*100);o.html(`
        ...
        `);return n.find(".acex-btn-details").on("click",()=>{n.find(".acex-history-details").slideToggle(200),n.toggleClass("expanded")}),n.find(".acex-btn-delete-google").on("click",()=>{this.deleteGoogleSheet(e.google_sheet_id,e.id,n)}),n.find(".acex-btn-delete").on("click",()=>{this.deleteHistoryRecord(e.id,n,e.google_sheet_id)}),n}renderDetails(e){let t='<div class="acex-details-content">';return e.errors&&e.errors.length>0&&(t+=`
			...
			`),t+="</div>",t}async renderGoogleSheetsList(e){let s=(await this.load()).records||[];P(".acex-google-sheets-list").remove();let a=P('<div class="acex-google-sheets-list">').appendTo(e);return console.log("Rendering Google Sheets List",this.history),s.forEach(i=>{if(i.google_sheet_url){let o=P(`
				...
				`);a.append(o)}}),P("body").off("click.acex-google-import",".actus-btn-import-sheet"),P("body").on("click.acex-google-import",".actus-btn-import-sheet",async i=>{i.preventDefault();let r=P(i.currentTarget).closest(".acex-google-sheet-item").data("id"),n=new x;n.showLoading("Loading data from Google Sheet..."),n.open();let c=await X.loadFile(r);n.close(),c&&fe.fileUploaded({file:c})}),a}async deleteGoogleSheet(e,t,s){if(!await x.confirm("Are you sure you want to delete this Google Sheet? This action cannot be undone.","Delete Google Sheet","Delete Google Sheet"))return;let i=new x;i.showLoading("Deleting Google Sheet..."),i.open();try{await X.deleteFile(e)?(await P.ajax({url:ajaxurl,method:"POST",data:{action:"acex_update_history_google",nonce:Y.nonce,id:t,google_sheet_id:null,google_sheet_url:null}}),s.find(".acex-btn-delete-google").remove(),s.find(".acex-history-google").remove(),i.close(),x.success("Google Sheet deleted successfully")):(i.close(),x.error("Failed to delete Google Sheet",result.error||"Unknown error"))}catch(o){i.close(),x.error("Error deleting Google Sheet",o.message)}}async deleteHistoryRecord(e,t,s=null){let a="Are you sure you want to delete this history record?";if(s&&(a+='<br><br><strong style="color: #d63638;">Warning:</strong> This record has an associated Google Sheet that will also be deleted permanently.'),!await x.confirm(a,"Delete History Record","Delete Record"+(s?" & Google Sheet":"")))return;let o=new x;o.showLoading("Deleting record..."),o.open();try{s&&await X.deleteFile(s);let r=await P.ajax({url:ajaxurl,method:"POST",data:{action:"acex_delete_history",nonce:Y.nonce,id:e}});o.close(),r.success?(t.fadeOut(300,()=>t.remove()),x.success("Record deleted successfully")):x.error("Failed to delete record",r.data)}catch(r){o.close(),x.error("Error deleting record",r.message)}}escapeHtml(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}async saveImport(e,t,s,a){let i=[],o=[];return t.results&&t.results.forEach((r,n)=>{r.error&&i.push(`Row ${n+3}: ${r.message}`),r.warnings&&r.warnings.length>0&&o.push(...r.warnings.map(c=>`Row ${n+3}: ${c}`))}),P.ajax({url:ajaxurl,method:"POST",data:{action:"acex_save_history",nonce:Y.nonce,history_data:{filename:e||"unknown.xlsx",action_type:"import",total_rows:s.length,successful:t.successful||0,failed:t.failed||0,created:t.created||0,updated:t.updated||0,errors:i,warnings:o,metadata:a}}})}async saveExport(e,t,s,a,i,o=null){let r={filename:e,action_type:"export",total_rows:t,exported:t,successful:t,failed:0,errors:[],warnings:[],metadata:{template_name:a?.name||"Export",format:o?"google":"xlsx",query:i||{},columns:a?.columns?.map(n=>n.key)||[],product_types:{simple:s?.simple_products||0,variable:s?.variable_products||0,variations:s?.variations||0,grouped:s?.grouped_products||0,external:s?.external_products||0,downloadable:s?.downloadable_products||0}}};return o&&o.sheetId&&(r.google_sheet_id=o.sheetId,r.google_sheet_url=`https://docs.google.com/spreadsheets/d/${o.sheetId}`),P.ajax({url:ajaxurl,method:"POST",data:{action:"acex_save_history",nonce:Y.nonce,history_data:r}})}},te=new Ye;var Ht=window.jQuery||jQuery,_t=window.acex_phpDATA||{},Ya=_t.strings||{},{__:$t}=wp.i18n,Ze=class{constructor(e,t){this.template=null,this.SelectProducts=null,this.DATA=_t,this.free=A.free}async export(e,t,s=!1){this.template=e,this.SelectProducts=t,this.check=await A.check();let a={};a=new x({title:"Export Products"});try{if(!this.template||!this.template.columns||this.template.columns.length===0){alert("Please select at least one column in the template before exporting.");return}let i=this.SelectProducts.query||{},o=await this.countQuery(a,i);if(o==="error")return;this.WB=new mt(this.template,i),this.workbook=await this.WB.createBase(),this.worksheet=await this.WB.createWorksheet();let r=await this.mainQuery(a,i,o),{allProducts:n,variations:c,simple_products:l,variable_products:u,grouped_products:h,external_products:p,downloadable_products:m}=r;a.showLoading($t("Creating Excel File...","actus-excel-for-woo")),await this.WB.finalize();let g=await this.downloadWorkbook(this.workbook),f;s&&(a.showLoading($t("Uploading to Google Sheets...","actus-excel-for-woo")),f=await X.uploadToGoogleSheets(this.workbook,g)),await te.saveExport(g,o,r,this.template,i,f),a.destroy(),this.check||o>LIMIT&&(o=LIMIT);let y={Template:this.template.name||"N/A","Products Exported":o};c&&(y["Products Exported"]+=` (+${c} variations)`),l&&(y["Simple Products"]=l),u&&(y["Variable Products"]=u),c&&(y.Variations=c),h&&(y["Grouped Products"]=h),p&&(y["External Products"]=p),m&&(y["Downloadable Products"]=m),f&&(y["Google Sheets Link"]=f.sheetId?`<a href="https://docs.google.com/spreadsheets/d/${f.sheetId}" target="_blank">Open Sheet</a>`:"-"),y["File Name"]=g,y.Format="Excel (.xlsx)",x.success("Export Successful!","Your products have been exported successfully.",y)}catch(i){a.destroy(),console.error("Export error:",i),x.error("Export Failed","An error occurred during the export process:",i.message,"Please try again or contact support if the problem persists.")}}async downloadWorkbook(e){let t=new Date().toISOString().slice(0,19).replace("T","_").replace(/-/g,"").replace(/:/g,""),s=`${this.template.name||"products"}_${t}.xlsx`,a=await e.xlsx.writeBuffer(),i=new Blob([a],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}),o=window.URL.createObjectURL(i),r=document.createElement("a");return r.href=o,r.download=s,r.click(),window.URL.revokeObjectURL(o),s}async countQuery(e,t){let s=await ne(t,!0);if(!s){e.open(),e.setContent("<p>No products found matching your filters.</p>"),e.updateButtons([{text:"Close",class:"actus-btn-primary",onClick:r=>r.destroy()}]);return}let a=`You are about to export ${s} products.`,i;return this.check||(a+=`<br><br>Your current license only allows exporting up to ${LIMIT} products.`,s>LIMIT&&(a+=` To export all ${s} products, please upgrade to the Pro version.`),i=[{text:"Upgrade Now",class:"actus-btn-primary",onClick:r=>{r.destroy(),window.open("https://awexcel.actusanima.com/","_blank")}},{class:"actus-modal-flex-1"}]),await x.confirm(a,"Export Products",i)?s:"error"}async mainQuery(e,t,s){this.check||s>LIMIT&&(s=LIMIT);let a=50,i=Math.ceil(s/a);e.updateTitle(`Export ${s} Products`),e.showLoading('Fetching...<br><div class="acex-progress"></div>'),e.open();let o=Ht(".actus-modal .acex-progress"),r=[];o.html(`0/${s} products`);let n=0,c=0,l=0,u=0,h=0,p=0,m=0;for(let g=1;g<=i;g++){let f=await ne(t,!1,a,g,!0);if(ae("export","batch",f),!f||!f.products)throw new Error(`Failed to fetch products page ${g}`);n+=f.simple_products||0,c+=f.variable_products||0,l+=f.variations||0,u+=f.grouped_products||0,h+=f.external_products||0,p+=f.downloadable_products||0,this.WB.addProductRows(Object.values(f.products)),m+=Object.values(f.products).length;let y=`${m-l}/${s} products`;l&&(y+=`<br> + ${l} variations`),o.html(y),f.products=null}return ae("export","fetched",{allProducts:r,simple_products:n,variable_products:c,variations:l,grouped_products:u,external_products:h,downloadable_products:p}),{allProducts:r,simple_products:n,variable_products:c,variations:l,grouped_products:u,external_products:h,downloadable_products:p}}},me=new Ze;var R=window.jQuery||jQuery,Rt=window.acex_phpDATA||{},oi=Rt.strings||{},{__:ge}=wp.i18n,Xe=class{constructor(){this.$mainContainer=R(".acex-main"),this.$buttonExportQuery=null,this.$buttonEditTemplate=null,this.$bodyContainer=null,this.$bodyTitle=null,this.$bodyNote=null,this.$bodyContainer=null}init(){this.isInitialized&&this.cleanup(),this.$mainContainer.empty(),this.exportNavigation(),this.exportBody(),Q.init(this.$bodyContainer),this.isInitialized=!0}cleanup(){R("body").off(".acex-export"),this.$navContainer=null,this.$bodyContainer=null,this.$bodyHeader=null,this.$bodyTitle=null,this.$bodyNote=null,this.$selectAllButton=null,this.isInitialized=!1,Q.cleanup()}exportNavigation(){this.$navContainer=R('<div class="acex-navigation">').appendTo(this.$mainContainer),this.$navContainer.on("click",".acex-navigation-button",function(){R(this).addClass("actus-active").siblings(".acex-navigation-button").removeClass("actus-active")}),this.$buttonExportQuery=R('<button class="acex-navigation-button">').text(ge("Export Sheet","actus-excel-for-woo")).appendTo(this.$navContainer),this.$buttonExportQuery.on("click",this.export.bind(this)),this.$buttonEditTemplate=R('<button class="acex-navigation-button">').text(ge("Edit Template","actus-excel-for-woo")).appendTo(this.$navContainer),this.$buttonEditTemplate.on("click",()=>{this.$bodyTitle.text(ge("Edit Template","actus-excel-for-woo")),this.$bodyNote.text(""),Q.edit()}),setTimeout(()=>{this.$buttonExportQuery.trigger("click")},0)}exportBody(){this.$bodyContainer=R('<div class="acex-export-body">').appendTo(this.$mainContainer),this.$bodyHeader=R('<div class="acex-header-flex">').appendTo(this.$bodyContainer),this.$bodyTitle=R("<h2>").appendTo(this.$bodyHeader),this.$bodyNote=R("<p>").appendTo(this.$bodyHeader),this.$bodyContainer=R('<div class="acex-export-body-container">').appendTo(this.$bodyContainer)}async export(){this.$bodyTitle.text(ge("Export","actus-excel-for-woo")),this.$bodyNote.text(ge("Export products to an Excel (.xlsx) file","actus-excel-for-woo")),this.$bodyContainer.empty(),await le.render(this.$bodyContainer,Q.template),Q.renderList(this.$bodyContainer),Q.renderPreview(),Q.$list.clone().prependTo(this.$bodyContainer),this.exportEvents()}exportEvents(){R("body").off("click.acex-export",".acex-button-export"),R("body").on("click.acex-export",".acex-button-export",async e=>{e.preventDefault(),await me.export(Q.template,le)}),R("body").off("click.acex-export",".acex-button-export-google"),R("body").on("click.acex-export",".acex-button-export-google",async e=>{e.preventDefault();let t=await X.OAuth("acex_google_ready");if(t&&t.success){if(t.message!=="Already connected"&&!await Modal.confirm("Export Products to Google Sheets","Google Connected"))return"error";await me.export(Q.template,le,!0)}})}},et=new Xe;var ce=window.jQuery||jQuery,Nt=window.acex_phpDATA||{},ui=Nt.strings||{},$={};$.activeComponents={exportSheet:null,importSheet:null};$.init=async()=>{$.navigation(),ce(".actus-admin-nav-button").first().trigger("click"),window.addEventListener("beforeunload",$.cleanupAll)};$.cleanupAll=()=>{console.log("Cleaning up all components..."),Object.keys($.activeComponents).forEach(d=>{let e=$.activeComponents[d];e&&typeof e.cleanup=="function"&&(console.log(`Cleaning up ${d}`),e.cleanup()),$.activeComponents[d]=null})};$.navigation=()=>{ce(".actus-admin-nav-button").on("click",function(){let d=ce(this).attr("alt");d!=="export"&&$.activeComponents.exportSheet&&(typeof $.activeComponents.exportSheet.cleanup=="function"&&$.activeComponents.exportSheet.cleanup(),$.activeComponents.exportSheet=null),d!=="import"&&$.activeComponents.importSheet&&(typeof $.activeComponents.importSheet.cleanup=="function"&&$.activeComponents.importSheet.cleanup(),$.activeComponents.importSheet=null),d!=="history"&&$.activeComponents.history&&(typeof $.activeComponents.history.cleanup=="function"&&$.activeComponents.history.cleanup(),$.activeComponents.history=null),d!=="license"&&$.activeComponents.license&&(typeof $.activeComponents.license.cleanup=="function"&&$.activeComponents.license.cleanup(),$.activeComponents.license=null),ce(".actus-admin-nav-button").removeClass("actus-active"),ce(this).addClass("actus-active"),d=="home"&&(document.querySelector(".actus-admin-main").innerHTML=""),d=="export"&&(et.init(),$.activeComponents.exportSheet=et),d=="import"&&(fe.init(),$.activeComponents.importSheet=fe),d=="history"&&(te.init(ce(".acex-main")),$.activeComponents.history=te),d=="license"&&(A.start(),$.activeComponents.license=A)})};window.addEventListener("beforeunload",()=>{Object.values($.activeComponents).forEach(d=>{d?.cleanup?.()})});var kt=$;var Ct=localStorage.getItem("acex_preferences");window.acex_phpDATA.prefs=Ct?JSON.parse(Ct):{};window.acex_phpDATA.prefs.save=()=>{let d={...window.acex_phpDATA.prefs};delete d.save,localStorage.setItem("acex_preferences",JSON.stringify(d)),console.warn("Preferences saved:",d)};var Tt=window.acex_phpDATA||{},Ci=Tt.strings||{};console.log("ACEX",Tt);Bt();async function Bt(){await A.init(),kt.init()}})();
